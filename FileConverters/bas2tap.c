  ...
  else
  {
    ...
    *WhichChannel = tolower (**Index);
    ...
    if (*WhichChannel == 'k' || *WhichChannel == 's' || *WhichChannel == 'p' ||                     /* (Normal Spectrum channels) */
        ...
        *WhichChannel == '#' || *WhichChannel == 0xCF || *WhichChannel == 'd')                                                         /* ('CAT' channel) */
      NeededHardware = 0;
        ...
        *WhichChannel == '#' || *WhichChannel == 0xCF)
    {
      ...
      if (!HandleClass06 (BasicLineNo, StatementNo, Keyword, Index))                                   /* Find numeric expression */
        return (FALSE);
      if (*WhichChannel == 'm' || *WhichChannel == 'd')                                        /* Omly the 'm' channel requires a ';' character following */
      {
        if (CheckEnd (BasicLineNo, StatementNo, Index))
          return (FALSE);
        if (**Index != ';')
  ...
  switch (Keyword)
  {
    ...
    case 0xD5: if (**Index == '*')                                                                                     /* (MERGE) */
               {
                 ...
                 if (!ScanChannel (BasicLineNo, StatementNo, Keyword, Index, &WhichChannel))
                   return (FALSE);
                 if (WhichChannel != 'm' && WhichChannel != 'b' && WhichChannel != 'n' && WhichChannel != 'd')
                 {
               ...
               if (WhichChannel != '\0' && WhichChannel != 'm' && WhichChannel != 'd')                      /* Not tape nor microdrive/disk channel ? */
               {
    ...
    case 0xD0: if (!ScanChannel (BasicLineNo, StatementNo, Keyword, Index, &WhichChannel))                            /* (FORMAT) */
               ...
               switch (WhichChannel)
               {
                 case 'm' : 
				 case 'd' : 
							if (CheckEnd (BasicLineNo, StatementNo, Index))         /* "m" requires an additional new volume name */
                              return (FALSE);
    ...
    case 0xD1: for (MoveLoop = 0 ; MoveLoop < 2 ; MoveLoop ++)                                                          /* (MOVE) */
               {
                 ...
                 else
                 {
                   ...
                   switch (WhichChannel)
                   {
                     case 'm' : 
					 case 'd' :
								if (CheckEnd (BasicLineNo, StatementNo, Index))            /* "m" requires an additional filename */
                                  return (FALSE);
                                if (**Index == '\"')                                                       /* Look for a filename */
                                {
                                  if (*(*Index + 1) == '\"' &&                                    /* Empty string (not allowed) ? */
...
//      exit (1);
    ...
    if (!Quiet)
    {
      ...
      switch (UsesInterface1)
      {
        ...
        case 0  : printf ("Note: this program requires Interface 1 or Opus Discovery or HC IF1\n"); break;
...
int bas2tap(char* basFilePath, const char* name, short unsigned autorunLine)
{
    ...
    char* args[] = { "", "-q", "-a0", nameArg, basFilePath, "temp.tap" };
    return mainOld(sizeof(args) / sizeof(args[0]), args);
}